...
async def _(pong):
    ...
    await kopong.edit(
        ...
        f"\n ★ 𝗬𝗼𝘂𝗿 𝗣𝗶𝗻𝗴 `%sms` \n"
        f"★ 𝗨𝗽𝘁𝗶𝗺𝗲 "
        f"\n ★ User - 𝗡𝗮𝘆𝗹𝗮『[{user.first_name}](tg://user?id={user.id})』 \n" % (duration)
    )
...
async def _(pong):
    ...
    await kping.edit("8===😬D🔥")
    await kping.edit("8====D🔥🔥")
    await kping.edit("**𝗡𝗮𝘆𝗹𝗮**")
    await kping.edit("**𝗡𝗮𝘆𝗹𝗮-𝗨𝘀𝗲𝗿𝗯𝗼𝘁 .....**")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await kping.edit("☠️")
    sleep(3)
...
CMD_HELP.update(
    {
        "ping": f"**Plugin : **`ping`\
        \n\n  »  **Perintah :** `{cmd}ping` ; `{cmd}lping` ; `{cmd}xping` ; `{cmd}nay`\
