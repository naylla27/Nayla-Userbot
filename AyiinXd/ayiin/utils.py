...
async def autopilot():
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://graph.org/file/84b5927d52cd79e8affee-b287e20624a524c87b.jpg", "photoyins.jpg"
        )
...
async def autobot():
    ...
    if who.username:
        ...
        username = f"nayla{(str(who.id))[5:]}bot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = f"nayla{str(who.id)[6:]}{ran}bot"
        await bot.send_message(bf, username)
        await asyncio.sleep(1)
        ...
        if nowdone.startswith("Done!"):
            ...
            await bot.send_message(
                BOTLOG_CHATID,
                "**restarting userbot, if there are any problems please contact @nangung!!**",
            )
        ...
        await bot.send_message(
            BOTLOG_CHATID,
            "**currently restarting userbot, if there are any problems please contact @mYual!**",
        )
        heroku_var["BOT_TOKEN"] = token
        heroku_var["BOT_USERNAME"] = f"@{username}"
    else:
